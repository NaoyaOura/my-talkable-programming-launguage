...
* PHPとは、「Hypertext Preprocessor（ピー・エイチ・ピー：ハイパーテキストプリプロセッサ）」の略
* Webアプリケーションのサーバーサイド・スクリプトに専門化している
* C言語やJava言語に似た、平易な文法とされるが、型の硬さなどはjavaなどと比較すると書きやすいが、やや冗長な文法
* ASP, JSPと似たHTML埋め込み型の構文をとる
* javaと同じようなクラス設計ができるが、クライアント側で動作することが多いことからも複雑なinterface設計は好まれない
